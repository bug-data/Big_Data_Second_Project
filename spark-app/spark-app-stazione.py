...
def read_json_file(x):
    ...
    return ((1),(data["id_dato"],data["data_ora"], 1, float(data["temp1_media"]), float(data["temp1_min"]), float(data["temp1_max"]), float(data["ur1_media"]), float(data["ur1_min"]), float(data["ur1_max"]), float(data["pioggia_mm"]), rad_Wmq, rad_Wmq_coun, float(data["wind_dir"]), wind_speed_avg, wind_speed_avg_count, wind_speed_max, wind_speed_max_count, float(data["pressione_mbar"]), float(data["pressione_standard_mbar"]) ))

def reduce_json(x,y):
    return (y[0],y[1],y[2]+x[2],y[3]+x[3], y[4]+x[4], y[5]+x[5], y[6]+x[6], y[7]+x[7], y[8]+x[8], y[9]+x[9], y[10]+x[10], y[11]+x[11], y[12]+x[12], y[13]+x[13], y[14]+x[14], y[15]+x[15], y[16]+x[16], y[17]+x[17], y[18]+x[18])
    ...
    json_body = [
        {
            ...
            "fields": {
                ...
                "ur1_media": (data[5]/data[2]),
                "ur1_min": (data[6]/data[2]),
                "ur1_max": (data[7]/data[2]),
                "pioggia_mm": (data[8]/data[2]),
                "rad W/mq": (data[10]/count_11),
